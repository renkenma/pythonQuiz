...
HINWEISE = {
    "Pop-Punk": [
        "Das Event findet drinnen statt – du brauchst keine Regenjacke.",
        "Es ist ein Musik-Event – live, laut und energiegeladen.",
        "Die Bands spielen nicht nur eigene Songs – Cover-Songs stehen auf dem Programm.",
        "Die Cover-Songs stammen von einer Künstlerin, die du sehr gut kennst.",
        "Der Name des Events hat 3 Wörter. Das erste Wort ist: 'Call'.",
    ],
    "Electro": [
    ...
    "Comedy": [
        "Dieser Abend wird dich garantiert zum Lachen bringen.",
        "Es ist kein Konzert – es wird geredet, nicht gesungen.",
        "Es findet in einer Stadt in Niedersachsen statt.",
        "Die Stadt liegt an der Hunte und ist bekannt für ihre Universität.",
        "Das Event heißt '3. ___ Nacht' – das mittlere Wort beschreibt den Humor.",
    ],
}

LOESUNGEN = {
    "Pop-Punk": "Call it Off",
    "Electro": "Strandfieber Festival",
    "Comedy": "3. Komische Nacht in Oldenburg",
}

SCHLUESSEL_WOERTER = {
    "Pop-Punk": ["call it off", "call", "callitoff"],
    "Electro": ["strandfieber", "strand fieber", "festival", "strand"],
    "Comedy": ["komische nacht", "oldenburg", "komisch", "komische"],
}

ERGEBNIS_TEXTE = {
    "Pop-Punk": {
        "titel": "🎸 Fast richtig! Hier kommt dein Geschenk:",
        "loesung_text": "**Call it Off** – Gitarren, Energie und Taylor Swift Covers lauthals mitsingen! Mach dich bereit für einen unvergesslichen Abend.",
        "typ": "success",
    },
    "Electro": {
        "titel": "🏖️ Fast richtig! Hier kommt dein Geschenk:",
        "loesung_text": "**Strandfieber Festival** – treibende Beats und Festival-Vibes! Pack die Tanzschuhe und die Sonnenbrille ein.",
        "typ": "warning",
    },
    "Comedy": {
        "titel": "😂 Fast richtig! Hier kommt dein Geschenk:",
        "loesung_text": "**3. Komische Nacht in Oldenburg** – ein genialer Comedy-Abend mit Lachkrampf-Garantie.",
        "typ": "success",
    },
}

RICHTIG_GERAET_TEXTE = {
    "Pop-Punk": "**Call it Off** – du hast es erraten! Gitarren, Energie und Taylor Swift Covers lauthals mitsingen. Mach dich bereit für einen unvergesslichen Abend! 🎸",
    "Electro": "**Strandfieber Festival** – du hast es erraten! Treibende Beats und Festival-Vibes warten auf dich. Pack die Tanzschuhe und die Sonnenbrille ein! 🏖️",
    "Comedy": "**3. Komische Nacht in Oldenburg** – du hast es erraten! Ein genialer Comedy-Abend mit Lachkrampf-Garantie wartet auf dich! 😂",
}


def hinweis_raetsel(sieger):
    st.divider()
    st.subheader("🔍 Weißt du schon, was es ist?")
    st.write("Ich verrate dir nicht sofort, was dein Geschenk ist – aber ich gebe dir Hinweise. Kannst du es erraten?")

    hinweise = HINWEISE[sieger]
    ...
    if st.session_state.erraten:
        st.success(f"🎉 Richtig geraten! {RICHTIG_GERAET_TEXTE[sieger]}")
        st.balloons()
        return

    if st.session_state.loesung_gezeigt:
        info = ERGEBNIS_TEXTE[sieger]
        st.markdown(f"### {info['titel']}")
        if info["typ"] == "warning":
            st.warning(info["loesung_text"])
        else:
            st.success(info["loesung_text"])
        st.info("Solltest du Interesse daran haben zu erfahren, was sich hinter den anderen Geschenken verbirgt, halte ich dich nicht davon ab dieses Quiz erneut zu spielen 😊! Vielleicht ist dein Charakter ja doch anders als in diesem Durchlauf 🤫.")
        st.balloons()
        return
    ...
    rateversuch = st.text_input(
        "Dein Tipp:",
        key=f"rateversuch_{sieger}_{aufgedeckt}",
        placeholder="Was könnte es sein...?"
    )

    col_rate, col_hinweis = st.columns([1, 1])
    ...
    with col_rate:
        if st.button("✅ Raten", use_container_width=True, key=f"btn_raten_{aufgedeckt}"):
            if rateversuch.strip():
                ...
                treffer = any(kw in tipp_lower for kw in SCHLUESSEL_WOERTER[sieger])
                if treffer:
    ...
    if st.session_state.falsche_versuche > 0 and not st.session_state.erraten:
        if aufgedeckt < len(hinweise):
            st.caption(f"Noch nicht ganz – ich habe dir einen neuen Hinweis aufgedeckt! ({len(hinweise) - aufgedeckt} weitere verfügbar)")
        else:
            st.caption("Alle Hinweise sind aufgedeckt. Du kannst es weiter versuchen oder dir die Lösung anzeigen lassen.")
...
# ==========================================
    ...
    if not st.session_state.tie_break_active and st.session_state.aktuelle_frage < len(fragen):
        ...
        if ist_gleichstand:
            ...
            st.warning("🤔 Kopf-an-Kopf-Rennen!")
            st.subheader("Welche dieser Fragen war für dich am wichtigsten?")
            ...
            if sieger == "Pop-Punk":
                ...
                st.warning(f"Dein Ergebnis ist: {sieger}")
            else:
                st.success(f"Dein Ergebnis ist: {sieger}")

            hinweis_raetsel(sieger)
